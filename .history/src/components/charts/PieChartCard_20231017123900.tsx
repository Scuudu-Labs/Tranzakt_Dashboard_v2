...
export default function PieChartCard(props: {
  ...
  return (
    <div className="w-[266px] border border-[#EAEAEA] rounded-[16px] px-[18px] min-h-[300px] bg-white  flex flex-col items-center justify-center">
      <div className="flex flex-col w-full">
        <p>{props.label}</p>
        <p className="text-xs text-[#2B2B2B]">{props.sublabel}</p>
      </div>
      <PieChart className="py-4" width={200} height={180}>
        <Pie
          data={props.data}
          dataKey="value"
          nameKey="name"

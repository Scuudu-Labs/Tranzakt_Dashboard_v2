  ...
  return (
    <div className="w-full h-full flex flex-col rounded-[16px] bg-white border-[1px] py-[16px] border-[#E3E3E3]">
        ...
        <div className='px-[16px] flex items-center py-[16px]'>
            <span className={`mr-2 ${state === "external" ? "text-[#32C87D]" : "text-[#A1A1A1]"} font-montserrat cursor-pointer`} onClick={()=> setState("externam")}>External</span>
            <span 
            className={`mr-2 ${state === "internal" ? "text-[#32C87D]" : "text-[#A1A1A1]"} font-montserrat cursor-pointer`}
            >Internal</span>
        </div>
  </div>
  )
